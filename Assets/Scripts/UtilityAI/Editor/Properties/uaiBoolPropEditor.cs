...
namespace JakePerry
{
    [CustomPropertyDrawer(typeof(uaiBoolProp))]
    public class uaiBoolPropEditor : PropertyDrawer
    {
        // Override height for each property
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            float linesPerProperty = 4.5f + ((property.FindPropertyRelative("_startRandom").boolValue) ? 1 : 0);
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            // Draw title
            EditorGUI.DropShadowLabel(new Rect(rect.x - 10, rect.y - 2, 100, singleLineHeight), "Bool Property");
